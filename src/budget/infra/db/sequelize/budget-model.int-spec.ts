...
describe("BudgetModel Integration Tests", () => {
  setupSequelize({ models: [BudgetModel] });

  test("mapping attributes", () => {
    const attributesMap = BudgetModel.getAttributes();
    const attributes = Object.keys(BudgetModel.getAttributes());
  ...
  test("create", async () => {
    ...
    const entity = await BudgetModel.create(arrange);
