...
import { TeamMemberRepository } from "#team-member/domain/repository/team-member-repository";
...
describe("TeamMemberSequelizeRepository Integration Tests", () => {
  setupSequelize({ models: [TeamMemberModel] });

  let repository: TeamMemberSequelize.TeamMemberRepository;

  beforeEach(async () => {
    repository = new TeamMemberSequelize.TeamMemberRepository(TeamMemberModel);
  });

  it("should insert a new entity", async () => {
    let entity = new TeamMember(
      { name: "new entity" },
      { created_by: "system" }
    );
    await repository.insert(entity);
    let model = await TeamMemberModel.findByPk(entity.id);
    ...
    await repository.insert(entity);
    model = await TeamMemberModel.findByPk(entity.id);
    expect(model.toJSON()).toStrictEqual(entity.toJSON());
  });
  ...
  it("should delete an entity", async () => {
    ...
    const foundEntity = await TeamMemberModel.findByPk(entity.id);
  ...
  it("should return search result", async () => {
    ...
    const result = await repository.search(
      new TeamMemberRepository.SearchParams({
        page: 1,
        per_page: 2,
        sort: "name",
        sort_dir: "asc",
  ...
  describe("search method", () => {
    it("should only apply paginate when other params are null ", async () => {
      ...
      const models = await TeamMemberModel.factory()
        .count(16)
        .bulkCreate(() => ({
      ...
      const spyToEntity = jest.spyOn(TeamMemberModelMapper, "toEntity");
      ...
      expect(result).toBeInstanceOf(TeamMemberRepository.SearchResult);
      ...
      result.items.forEach((item) => {
        expect(item).toBeInstanceOf(TeamMember);
      ...
      expect(JSON.stringify(result)).toStrictEqual(
        JSON.stringify(
          new TeamMemberRepository.SearchResult({
            items: entities,
            total: 16,
            current_page: 1,
            per_page: 15,
    ...
    it("should apply paginate and filter", async () => {
      ...
      const entities = await TeamMemberModel.bulkCreate(entitiesProps);
      ...
      let searchOutputExpected = new TeamMemberRepository.SearchResult({
        items: [
          TeamMemberModelMapper.toEntity(entities[0]),
          TeamMemberModelMapper.toEntity(entities[2]),
        ],
        total: 3,
        current_page: 1,
        per_page: 2,
      ...
      searchOutputExpected = new TeamMemberRepository.SearchResult({
        items: [TeamMemberModelMapper.toEntity(entities[3])],
        total: 3,
        current_page: 2,
        per_page: 2,
        sort: null,
    ...
    it("should apply paginate and sort", async () => {
      ...
      const models = await TeamMemberModel.bulkCreate(entitiesProps);
      ...
      const arrange = [
        {
          params: new TeamMemberRepository.SearchParams({
            page: 1,
            per_page: 2,
            sort: "name",
          }),
          result: new TeamMemberRepository.SearchResult({
    ...
    describe("should search using filter, sort and paginate", () => {
      ...
      beforeEach(async () => {
        await TeamMemberModel.bulkCreate(entitiesProps);
      });

      const arrange = [
        {
          search_params: new TeamMemberRepository.SearchParams({
            page: 1,
            per_page: 2,
            sort: "name",
            sort_dir: "asc",
          ...
          search_result: new TeamMemberRepository.SearchResult({
      ...
      function toEntity(entityProps: any): TeamMember {
