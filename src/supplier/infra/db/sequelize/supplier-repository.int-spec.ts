...
import { SupplierRepository } from "#supplier/domain/repository/supplier-repository";
...
describe("SupplierSequelizeRepository Integration Tests", () => {
  setupSequelize({ models: [SupplierModel] });

  let repository: SupplierSequelize.SupplierRepository;

  beforeEach(async () => {
    repository = new SupplierSequelize.SupplierRepository(SupplierModel);
  });

  it("should insert a new entity", async () => {
    let entity = new Supplier({ name: "new entity" }, { created_by: "system" });
    await repository.insert(entity);
    let model = await SupplierModel.findByPk(entity.id);
    ...
    await repository.insert(entity);
    model = await SupplierModel.findByPk(entity.id);
    expect(model.toJSON()).toStrictEqual(entity.toJSON());
  });
  ...
  it("should delete an entity", async () => {
    ...
    const foundEntity = await SupplierModel.findByPk(entity.id);
  ...
  it("should return search result", async () => {
    ...
    const result = await repository.search(
      new SupplierRepository.SearchParams({
        page: 1,
        per_page: 2,
        sort: "name",
        sort_dir: "asc",
  ...
  describe("search method", () => {
    it("should only apply paginate when other params are null ", async () => {
      ...
      const models = await SupplierModel.factory()
        .count(16)
        .bulkCreate(() => ({
      ...
      const spyToEntity = jest.spyOn(SupplierModelMapper, "toEntity");
      ...
      expect(result).toBeInstanceOf(SupplierRepository.SearchResult);
      ...
      result.items.forEach((item) => {
        expect(item).toBeInstanceOf(Supplier);
      ...
      expect(JSON.stringify(result)).toStrictEqual(
        JSON.stringify(
          new SupplierRepository.SearchResult({
            items: entities,
            total: 16,
            current_page: 1,
            per_page: 15,
    ...
    it("should apply paginate and filter", async () => {
      ...
      const entities = await SupplierModel.bulkCreate(entitiesProps);
      ...
      let searchOutputExpected = new SupplierRepository.SearchResult({
        items: [
          SupplierModelMapper.toEntity(entities[0]),
          SupplierModelMapper.toEntity(entities[2]),
        ],
        total: 3,
        current_page: 1,
        per_page: 2,
      ...
      searchOutputExpected = new SupplierRepository.SearchResult({
        items: [SupplierModelMapper.toEntity(entities[3])],
        total: 3,
        current_page: 2,
        per_page: 2,
        sort: null,
    ...
    it("should apply paginate and sort", async () => {
      ...
      const models = await SupplierModel.bulkCreate(entitiesProps);
      const items = models.map((model) => SupplierModelMapper.toEntity(model));

      const arrange = [
        {
          params: new SupplierRepository.SearchParams({
            page: 1,
            per_page: 2,
            sort: "name",
          }),
          result: new SupplierRepository.SearchResult({
    ...
    describe("should search using filter, sort and paginate", () => {
      ...
      beforeEach(async () => {
        await SupplierModel.bulkCreate(entitiesProps);
      });

      const arrange = [
        {
          search_params: new SupplierRepository.SearchParams({
            page: 1,
            per_page: 2,
            sort: "name",
            sort_dir: "asc",
          ...
          search_result: new SupplierRepository.SearchResult({
      ...
      function toEntity(entityProps: any): Supplier {
        return new Supplier(
