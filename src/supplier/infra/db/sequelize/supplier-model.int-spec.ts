...
describe("SupplierModel Integration Tests", () => {
  setupSequelize({ models: [SupplierModel] });

  test("mapping attributes", () => {
    const attributesMap = SupplierModel.getAttributes();
    const attributes = Object.keys(SupplierModel.getAttributes());
  ...
  test("create", async () => {
    ...
    const entity = await SupplierModel.create(arrange);
